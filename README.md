...
When you run the script, choose one of:

1. **Create a new staging site** — pick a live site, enter a staging domain, full provision + clone.
2. **Remove a staging site** — pick a staging site, type its domain name to confirm deletion.
3. **Refresh a staging site from live** — pick only the staging site; the script finds the live source when possible.

Staging sites are identified by domain names containing `staging` or `studiorepublic` (safe targets for refresh/remove).

## Supported stacks

The script detects the site type from the document root after files are copied:

| Detection | Type |
|-----------|------|
| `wp-config.php` present | WordPress |
| `artisan` + `.env` present | Laravel |
| `.env` only (no `wp-config.php`) | Laravel (fallback) |
| Neither | Unknown (files + DB only; warning printed) |

### WordPress staging environment

On **create** and **refresh**:

- Updates staging DB credentials and `WP_HOME` / `WP_SITEURL` in `wp-config.php` (create only for DB/URL defines).
- Sets `WP_ENV` and `WP_ENVIRONMENT_TYPE` to `local` in `wp-config.php`.
- Runs WP-CLI `search-replace` for live → staging URLs (http and https; network mode for multisite).

### Laravel staging environment

On **create** and **refresh** (`.env` is patched in place; never overwritten wholesale on refresh):
...
| `DB_DATABASE` / `DB_USERNAME` / `DB_PASSWORD` | Staging credentials (create: new DB; refresh: read from preserved `.env`) |
| `MAIL_MAILER` | `log` (create always; refresh only if key already exists) |
...
3. Provisions site, clones DB/files, applies stack-specific config (WordPress or Laravel), Varnish, vHost; optional SSL.

### Remove staging site

1. Pick a staging site from the filtered list.
2. Review domain, user, and database name.
3. **Type the staging domain exactly** to confirm permanent deletion (the script auto-confirms CloudPanel’s internal `yes/no` prompts for `clpctl` delete).
...
Use the arrow keys to select an action from the main menu, then follow the prompts. For refresh, you only pick the staging site (live is auto-detected when possible). For remove, type the staging domain to confirm deletion.

*(Note: For new sites, point DNS A-records for the staging domain to your server before issuing SSL.)*

## 🧹 Cleaning Up (Destroying the Staging Site)

Once you are done testing, you can remove a staging site from the script’s **Remove a staging site** menu (type the domain to confirm), or use the CloudPanel GUI / CLI:

You can also delete manually via the command line (replace `stg.example.com` and `database_name` with your staging values):
